...
struct AudioBufferUtils {
    ...
    MC_NODISCARD static auto createNoiseBuffer(int channels, int size, FloatT gain = 1.0) -> juce::AudioBuffer<FloatT>;
    ...
    MC_NODISCARD static auto containsNANs(juce::AudioBuffer<float> const& buffer) noexcept -> bool;
    MC_NODISCARD static auto containsNANs(juce::AudioBuffer<double> const& buffer) noexcept -> bool;

    MC_NODISCARD static auto containsINFs(juce::AudioBuffer<float> const& buffer) noexcept -> bool;
    MC_NODISCARD static auto containsINFs(juce::AudioBuffer<double> const& buffer) noexcept -> bool;

    MC_NODISCARD static auto allOf(juce::AudioBuffer<float> const& buffer, float value) noexcept -> bool;
    MC_NODISCARD static auto allOf(juce::AudioBuffer<double> const& buffer, double value) noexcept -> bool;

    MC_NODISCARD static auto equal(juce::AudioBuffer<float> const& lhs, juce::AudioBuffer<float> const& rhs) -> bool;
    MC_NODISCARD static auto equal(juce::AudioBuffer<double> const& lhs, juce::AudioBuffer<double> const& rhs) -> bool;
};

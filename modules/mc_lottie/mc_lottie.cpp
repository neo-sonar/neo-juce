#include "mc_lottie.hpp"

#include "animation/lottie_animation.cpp"         // NOLINT(bugprone-suspicious-include)
#include "layer/lottie_layer.cpp"                 // NOLINT(bugprone-suspicious-include)
#include "layer/lottie_layer_shape.cpp"           // NOLINT(bugprone-suspicious-include)
#include "model/lottie_model.cpp"                 // NOLINT(bugprone-suspicious-include)
#include "shape/lottie_shape.cpp"                 // NOLINT(bugprone-suspicious-include)
#include "shape/lottie_shape_ellipse.cpp"         // NOLINT(bugprone-suspicious-include)
#include "shape/lottie_shape_fill.cpp"            // NOLINT(bugprone-suspicious-include)
#include "shape/lottie_shape_gradient_fill.cpp"   // NOLINT(bugprone-suspicious-include)
#include "shape/lottie_shape_gradient_stroke.cpp" // NOLINT(bugprone-suspicious-include)
#include "shape/lottie_shape_group.cpp"           // NOLINT(bugprone-suspicious-include)
#include "shape/lottie_shape_path.cpp"            // NOLINT(bugprone-suspicious-include)
#include "shape/lottie_shape_rectangle.cpp"       // NOLINT(bugprone-suspicious-include)
#include "shape/lottie_shape_transform.cpp"       // NOLINT(bugprone-suspicious-include)
#include "shape/lottie_shape_trim.cpp"            // NOLINT(bugprone-suspicious-include)

...
public:
     ...
     */
    [[nodiscard]] auto getMaxLevel(const int channel) const -> float;
     ...
     */
    [[nodiscard]] auto getRMSLevel(const int channel) const -> float;

    /**
     Returns the status of the clip flag.
     */
    [[nodiscard]] auto getClipFlag(const int channel) const -> bool;
...
template <typename FloatType>
void LevelMeterSource::measureBlock(const juce::AudioBuffer<FloatType>& buffer)
{
